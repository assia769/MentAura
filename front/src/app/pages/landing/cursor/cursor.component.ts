...
@Component({
  ...
  template: `
    <div class="cursor-aura"
         [style.left.px]="ax" [style.top.px]="ay"></div>

    <div class="cursor-glow"
         [style.left.px]="gx" [style.top.px]="gy"></div>
  ...
  styles: [`
    ...
    .cursor-ring {
      ...
      will-change: left, top;
      transition:
        left   .12s cubic-bezier(.22,1,.36,1),
        top    .12s cubic-bezier(.22,1,.36,1),
    ...
    .ring-arc {
      ...
      filter: drop-shadow(0 0 3px rgba(201,168,76,.9))
              drop-shadow(0 0 8px rgba(201,168,76,.5));
    ...
    .cursor-dot {
      ...
      background: #c9a84c;
      box-shadow:
        0 0  4px 1px rgba(201,168,76,.9),
        0 0 12px 3px rgba(201,168,76,.5),
        0 0 24px 6px rgba(201,168,76,.2);
      transform: translate(-50%, -50%);
      pointer-events: none;
      will-change: left, top;
      transition:
...
export class CursorComponent implements OnInit, OnDestroy {
  ...
  /* coordonnées à différents "poids" pour le parallax */
  x = -300; y = -300           // dot (instant)
  rx = -300; ry = -300         // ring (léger lag)
  gx = -300; gy = -300         // glow proche
  ax = -300; ay = -300         // aura lointaine

  hovered  = false
  clicking = false
  dashOffset = 28  // arc ≈ 75 % du cercle (113 - 0.75*113 ≈ 28)
  ...
  ngOnInit() { this.injectSvgDefs(); this.loop() }
  ...
  onDown() { this.clicking = true;  this.cdr.markForCheck() }

  @HostListener('document:mouseup')
  onUp()   { this.clicking = false; this.cdr.markForCheck() }
